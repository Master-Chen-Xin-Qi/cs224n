...
argp.add_argument('function',
    ...
    choices=["pretrain", "finetune", "evaluate"], default='evaluate')
argp.add_argument('variant',
    help="Which variant of the model to run ('vanilla' or 'synthesizer')",
    choices=["vanilla", "synthesizer"], default='vanilla')
argp.add_argument('pretrain_corpus_path',
    help="Path of the corpus to pretrain on", default='wiki.txt')
argp.add_argument('--reading_params_path',
    help="If specified, path of the model to load before finetuning/evaluation",
    default='vanilla.model.params')
argp.add_argument('--writing_params_path',
    help="Path to save the model after pretraining/finetuning", default='vanilla.model.params')
argp.add_argument('--finetune_corpus_path',
    help="Path of the corpus to finetune on", default='birth_places_train.tsv')
argp.add_argument('--eval_corpus_path',
    help="Path of the corpus to evaluate on", default='birth_dev.tsv')
argp.add_argument('--outputs_path', default='vanilla.nopretrain.dev.predictions')
...
if args.function == 'pretrain':
    ...
    tconfig = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3, lr_decay=True, warmup_tokens=512*20,
    ...
    fintune_dataset = dataset.NameDataset(pretrain_dataset, text)
    tconfig = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=6e-4, lr_decay=True,
        warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_works=4)
    trainer = trainer.Trainer(model, fintune_dataset, None, tconfig)
