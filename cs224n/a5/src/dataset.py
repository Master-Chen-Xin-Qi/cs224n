...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate_len = min(truncate_len, line_len)
        truncate_line = line[:truncate_len]
        mask_len = random.randint(int(1/8*truncate_len), int(3/8*truncate_len))
        prefix_len = random.randint(1, truncate_len-mask_len-1)
        prefix = truncate_line[:prefix_len]
        mask = truncate_line[prefix_len:prefix_len+mask_len]
        suffix = truncate_line[prefix_len+mask_len:]
        input_d = prefix+self.MASK_CHAR+suffix+self.MASK_CHAR+mask
        input_d = input_d+self.PAD_CHAR*(self.block_size-len(input_d))
        x = input_d[:-1]
        y = input_d[1:]
        x = torch.LongTensor([self.stoi[i] for i in x])
        y = torch.LongTensor([self.stoi[i] for i in y])
