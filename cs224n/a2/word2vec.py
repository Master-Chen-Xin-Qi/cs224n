...
def negSamplingLossAndGradient(
    ...
    scores = (outsideVectors[indices] @ centerWordVec)
    scores = np.expand_dims(scores, axis=1)
    true_score = scores[0]
    true_prob = sigmoid(true_score)
    negtive_score = -scores[1:]
    negative_prob = sigmoid(negtive_score)
    loss = -np.log(true_prob) - np.sum(np.log(negative_prob), axis=0)

    gradFromCorpus = -outsideVectors[outsideWordIdx] * (1 - true_prob)
    gradNotFromCorpus = np.sum(
        outsideVectors[negSampleWordIndices] * (1 - negative_prob), axis=0)
    gradCenterVec = gradFromCorpus + gradNotFromCorpus

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - true_prob)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    current_word_index = word2Ind[currentCenterWord]
    current_word_vector = centerWordVectors[current_word_index]
    for outsideword in outsideWords:
        outside_word_index = word2Ind[outsideword]
        currLoss, currGradCenter, currGradOutside = word2vecLossAndGradient(
            current_word_vector, outside_word_index, outsideVectors, dataset)
        loss += currLoss
        gradCenterVecs += currGradCenter
        gradOutsideVectors += currGradOutside
    gradCenterVecs[np.arange(gradCenterVecs.shape[0]) != current_word_index] = 0
    ### END YOUR CODE
