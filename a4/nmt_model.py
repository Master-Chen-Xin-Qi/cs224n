...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, len_unpacked = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.transpose(0, 1)
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0].squeeze(0), last_hidden[1].squeeze(0)), dim=1))
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0].squeeze(0), last_cell[1].squeeze(0)), dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            dec_state, o_state, c_state = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_state)
            o_prev = o_state
